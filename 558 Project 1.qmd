---
title: "558 Project 1"
author: "Susan H. and Holly P."
format: pdf
editor: visual
---

Load in necessary libraries:

```{r}
#| warning: FALSE
#| message: FALSE
library(tidyverse)
library(readr)


```

Read in data using read_csv():

```{r}
#| warning: FALSE
#| message: FALSE
census_data <- read_csv("EDU01a.csv")


```

## Question 1

```{r}

census_data1 <- census_data |> select(Area_name, STCOU, ends_with("D")) 

head(census_data1, 5)


```

## Question 2

```{r}
census_long <- census_data1 |> pivot_longer(cols = ends_with("D"), names_to = "EnrollmentID", values_to = "Enrollment_Total")
head(census_long,5)
```

## Question 3

```{r}
 #no dates above 1996
long_updated <- census_long |> mutate( Year = as.numeric(substr(EnrollmentID, start = 8, stop = 9))) |> 
mutate(Year = Year + 1900) |> 
mutate(Measurement = substr(EnrollmentID, start = 1, stop = 7) )

head(long_updated, 5)

```

## Question 4

```{r}
#State Dataset (not finished)
row_names <- rownames(long_updated)
state_subset <- row_names %in% grep(pattern = ", \\w\\w", long_updated$Area_name)
state_data <- subset(long_updated, county_subset)
class(county_data) <- c("county", class(county_data))
head(state_data, 10)

#County Dataset
county_subset <- row_names %in% grep(pattern = ", \\w\\w", long_updated$Area_name)
county_data <- subset(long_updated, county_subset)
class(county_data) <- c("county", class(county_data))
head(county_data, 10)
```

